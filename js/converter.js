...
$(window).on("load", () => {
  ...
  for (i in games) {
    ...
    var option = $(`<option>${games[i][1]}</option>`).attr(
      "value",
      `${games[i][0]}`
    );
    ...
    var result_custom = $(`<td>0.0000</td>`).attr(
      "id",
      `result-${games[i][0]}-custom`
    );
...
function calcSens() {
  ...
  if (dpi_output == "" || dpi_output <= 0) {
    ...
    var result_custom_header = $(
      `<th>SENSITIVITY @ ${dpi_output} DPI</th>`
    ).attr("colspan", `${games.length}`);
  ...
  $("#result-csgo-custom").text(
    (sens_csgo / dpi_output).toFixed(decimal_precision)
  );

  // convert to apex legends
  var sens_apex_legends = parseFloat(sens_source);
  $("#result-apex-legends").text(
    (sens_apex_legends / dpi_input).toFixed(decimal_precision)
  );
  $("#result-apex-legends-custom").text(
    (sens_apex_legends / dpi_output).toFixed(decimal_precision)
  );

  // convert to valorant
  var sens_valorant = parseFloat((sens_source * 22) / 7 / 10);
  $("#result-valorant").text(
    (sens_valorant / dpi_input).toFixed(decimal_precision)
  );
  $("#result-valorant-custom").text(
    (sens_valorant / dpi_output).toFixed(decimal_precision)
  );

  // convert to modernwarfare
  var sens_modern_warfare = parseFloat(sens_source * (1 / 3) * 10);
  $("#result-modern-warfare").text(
    (sens_modern_warfare / dpi_input).toFixed(decimal_precision)
  );
  $("#result-modern-warfare-custom").text(
    (sens_modern_warfare / dpi_output).toFixed(decimal_precision)
  );

  // convert to overwatch
  var sens_overwatch = parseFloat(sens_source * (1 / 3) * 10);
  $("#result-overwatch").text(
    (sens_overwatch / dpi_input).toFixed(decimal_precision)
  );
  $("#result-overwatch-custom").text(
    (sens_overwatch / dpi_output).toFixed(decimal_precision)
  );

  // convert to siege
  var sens_siege = parseFloat(sens_source * 3.83972336439);
  $("#result-siege").text((sens_siege / dpi_input).toFixed(decimal_precision));
  $("#result-siege-custom").text(
    (sens_siege / dpi_output).toFixed(decimal_precision)
  );

  // convert to hyper scape
  var sens_hyper_scape = parseFloat(sens_source * 3.83972336439);
  $("#result-hyper-scape").text(
    (sens_hyper_scape / dpi_input).toFixed(decimal_precision)
  );
  $("#result-hyper-scape-custom").text(
    (sens_hyper_scape / dpi_output).toFixed(decimal_precision)
  );
}
